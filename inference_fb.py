...
if __name__ == "__main__":
    ...
    unsatisfied_word = config.get('condition words', 'NOTSATISFIED')
    unsatisfied_word = list(unsatisfied_word.split(",")) 
    fashion_attributes = getAttributes() 

    # QAs
    while True:
        question = input("\n> ")
        KEYWORD = getKeyWords(question)
        # print("keywords:{}".format(KEYWORD))
        keyword_fashion = list(set(KEYWORD).intersection(fashion_attributes))
        keyword_unhappy = list(set(KEYWORD).intersection(unsatisfied_word))
        keyword_wordnet = list(set(KEYWORD).intersection(wordnet.words()))
        # print("keyword_fashion:{}".format(keyword_fashion))
        # print("keyword_unhappy:{}".format(keyword_unhappy))
        # print("keyword_wordnet:{}".format(keyword_wordnet))

        if len(keyword_fashion) != 0:
                generateImage(keyword_fashion[0])
        elif len(keyword_unhappy) != 0:
                giveHint()
        elif len(keyword_wordnet) != 0:
                allSynset = []
                for i in keyword_wordnet:
                    for j in wordnet.synsets(i):
                        for k in j.lemmas():
                            allSynset.append(k.name())
                keyword_FashionNet =  list(set(allSynset).intersection(fashion_attributes))
                if len(keyword_FashionNet) != 0:
                    generateImage(keyword_FashionNet[0])
